...
class UniformMatcher(nn.Module):
    ...
    def forward(self, pred_boxes, anchors, gt_boxes, gt_labels):
        # pred_boxes = torch.cat(pred_boxes, dim=1).view(-1, 4)
        ...
        tgt_bbox = torch.cat([v for v in gt_boxes])

        # Compute the L1 cost between boxes
        # Note that we use anchors and predict boxes both
        # cost_bbox = torch.cdist(box_utils.corner_form_to_center_form(out_bbox), box_utils.corner_form_to_center_form(tgt_bbox), p=1)
        # cost_bbox_anchors = torch.cdist(box_utils.corner_form_to_center_form(anchors), box_utils.corner_form_to_center_form(tgt_bbox), p=1)
